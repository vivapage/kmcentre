<?php
/**
 * The template for displaying all pages
 *
 * This is the template that displays all pages by default.
 * Please note that this is the WordPress construct of pages
 * and that other 'pages' on your WordPress site may use a
 * different template.
 *
 * @link https://developer.wordpress.org/themes/basics/template-hierarchy/
 *
 * @package kmcentre
 */

get_header();

?>

<main id="primary" class="site-main">
<div class="overflow_hidden">
	<div class="radius_niz_mini">
<!-- slider-top -->
		<div class="top-slider">
			<?php add_revslider('slider-1'); ?>
		</div>
<!-- /slider-top -->
	</div>
</div>

<!-- services -->
<div class=" row services">
	<div class="overflow_hidden">
		<div class="radius_row_niz services_row">
			<div class="services__container _container">
			<div class="services__container _slogan"><?php esc_html_e( 'Медичний центр, що пропонує Вам нову якість життя і дещо важливіше', 'kmcentre' ); ?></div>

			<div class="slider-nav">
						<div class= "slider-nav__item">
							<div class="slider-nav__item_img">
								<a class="active" data-slide-index="0"><svg height="75" viewBox="0 0 128 128" width="75" xmlns="http://www.w3.org/2000/svg"><g><path d="m105.652 51.574a1.752 1.752 0 0 0 -2.045 1.4 72.37 72.37 0 0 1 -3.522 12.613 1.75 1.75 0 1 0 3.268 1.252 76.042 76.042 0 0 0 3.694-13.215 1.75 1.75 0 0 0 -1.395-2.05z"/><path d="m122.191 17.392-8.1-3.481-3.483-8.1a1.749 1.749 0 0 0 -3.214 0l-1.724 4.009a19.973 19.973 0 0 0 -13.98-5.07c-9.3 0-16.413 5.275-20.949 9.24a10.314 10.314 0 0 1 -13.48 0c-4.547-3.966-11.661-9.24-20.951-9.24-6.486 0-11.729 2.25-15.584 6.69-6.681 7.69-8.675 21.635-5.941 41.436l-.875 2.035-8.1 3.481a1.75 1.75 0 0 0 0 3.216l8.1 3.481 3.483 8.1a1.749 1.749 0 0 0 3.214 0l.091-.212c1.566 3.492 3.191 6.467 4.67 9.172 3.149 5.761 5.632 10.309 5.914 16.629.946 21.293 9.433 24.472 14.408 24.472 7.881 0 8.9-10.044 9.893-19.758 1.133-11.126 2.4-19 8.417-19s7.282 7.874 8.413 19c.988 9.717 2.01 19.764 9.9 19.764 4.971 0 13.453-3.179 14.4-24.473.276-6.317 2.764-10.865 5.913-16.623 3.51-6.418 7.879-14.4 10.359-27.607 1.724-11.58 1.87-21.3.458-28.941l.652-1.517 8.1-3.481a1.75 1.75 0 0 0 0-3.216zm-105.341 45.674a1.758 1.758 0 0 0 -.916-.917l-5-2.149 5-2.149a1.758 1.758 0 0 0 .916-.917l2.15-5 2.15 5a1.758 1.758 0 0 0 .916.917l5 2.149-5 2.149a1.758 1.758 0 0 0 -.916.917l-2.015 4.689c-.01.021-.018.041-.027.062l-.108.251zm92.68-9.1c-2.367 12.59-6.412 19.984-9.979 26.507-3.235 5.914-6.028 11.02-6.339 18.149-.351 7.89-2.233 21.127-10.9 21.127-4.4 0-5.394-6.58-6.414-16.618-1.058-10.377-2.254-22.141-11.898-22.141s-10.843 11.766-11.9 22.148c-1.022 10.034-2.02 16.612-6.411 16.612-8.678 0-10.561-13.237-10.912-21.126-.313-7.132-3.105-12.239-6.34-18.152a104.388 104.388 0 0 1 -5.822-11.955l1.474-3.428 8.1-3.481a1.75 1.75 0 0 0 0-3.216l-8.1-3.481-3.483-8.1a1.732 1.732 0 0 0 -3.043-.278c-1.481-15.622.46-26.644 5.8-32.8 3.166-3.633 7.521-5.483 12.947-5.483 8.109 0 14.527 4.782 18.652 8.38a13.831 13.831 0 0 0 18.079 0c4.115-3.6 10.524-8.378 18.649-8.378 5.18 0 9.367 1.7 12.494 5.023l-.274.638-8.1 3.481a1.75 1.75 0 0 0 0 3.216l8.1 3.481 3.483 8.1a1.749 1.749 0 0 0 3.214 0l.11-.254a101.882 101.882 0 0 1 -1.187 22.03zm2.536-32.818a1.758 1.758 0 0 0 -.916.917l-2.15 5-2.15-5a1.758 1.758 0 0 0 -.916-.917l-5-2.149 5-2.149a1.758 1.758 0 0 0 .916-.917l2.15-5 2.15 5a1.758 1.758 0 0 0 .916.917l5 2.149z"/></g></svg><br><?php esc_html_e( 'СТОМАТОЛОГІЯ', 'kmcentre' ); ?></a>
							</div>
						</div>
						<div class= "slider-nav__item">
							<div class="slider-nav__item_img">
								<a class="active" data-slide-index="1"><svg height="75" viewBox="0 0 128 128" width="75" xmlns="http://www.w3.org/2000/svg"><path d="m124.256 61.859a12.645 12.645 0 0 0 -9-3.732h-17.777l.009-15.56a24.77 24.77 0 0 0 -15.679-22.98c-3.193-1.258-7.216-2.259-10.643-1.525a12.705 12.705 0 1 0 -7.983 8.638l-7.2 12.251-15.953 7.868a8.158 8.158 0 0 0 -4.259 5.308h-3.723a10.2 10.2 0 0 0 -7.567 3.365 12.649 12.649 0 1 0 -20.543 13.625h-2.2a1.75 1.75 0 0 0 -1.75 1.75v14.94a1.75 1.75 0 0 0 1.75 1.75h7v36.57a1.75 1.75 0 0 0 1.75 1.75h12a1.75 1.75 0 0 0 1.75-1.75v-20.527c.124.007.249.02.372.02a7.1 7.1 0 0 0 6.49-4.22l5.153-11.838h24.774l-2.581 3.408a7.768 7.768 0 0 0 -2.207 5.412v21.5a7.939 7.939 0 0 0 7.508 8h.236a7.761 7.761 0 0 0 7.756-7.753v-18.523l5.578-5.527-.938 4.821a8.059 8.059 0 0 0 -.078 2.48l2.87 17.681a7.755 7.755 0 0 0 7.688 6.819 7.885 7.885 0 0 0 .94-.053 7.759 7.759 0 0 0 6.757-8.628c0-.024-.007-.048-.011-.073l-2.71-16.5 2.545-13.069h9.358v36.57a1.75 1.75 0 0 0 1.75 1.75h12a1.75 1.75 0 0 0 1.75-1.75v-36.57h7a1.75 1.75 0 0 0 1.75-1.75v-14.94a12.66 12.66 0 0 0 -3.732-9.008zm-65.303-37.919a9.16 9.16 0 1 1 9.159-9.16 9.17 9.17 0 0 1 -9.159 9.16zm-17.371 26.016 16.418-8.1a1.75 1.75 0 0 0 .734-.683l10.4-17.7a4.26 4.26 0 0 1 3.2-2.065 2.948 2.948 0 0 1 .464-.028 21.21 21.21 0 0 1 21.19 21.189l-.009 15.561h-.149a23.665 23.665 0 0 0 -13.293-5.869l.932-6.113a11.6 11.6 0 0 0 -6.752-12.327 1.749 1.749 0 0 0 -2.218.712l-7.352 12.5a4.226 4.226 0 0 1 -1.784 1.653l-18.25 9a4.249 4.249 0 0 1 -5.743-2.046 3.762 3.762 0 0 1 -.342-1.732 4.572 4.572 0 0 1 2.554-3.952zm36.426-4.339-.992 6.51h-12.716l.612-.3a7.714 7.714 0 0 0 3.255-3.027l6.482-11.025a8.1 8.1 0 0 1 3.359 7.842zm-74.52 14.336a9.16 9.16 0 1 1 9.159 9.16 9.169 9.169 0 0 1 -9.159-9.16zm18.319 8.714v.45h-.45q.231-.217.451-.45zm-11.353 15.39h-6.966v-11.44h18.32v11.436zm10.284 3.835-2.478 5.686a7.37 7.37 0 0 0 .362 6.717 6.689 6.689 0 0 0 2.117 2.184v19.9h-8.5v-34.822h8.5zm14.37-18.775a1.75 1.75 0 0 0 -1.75 1.75v14.575l-5.458 12.552a3.548 3.548 0 0 1 -4.052 2.044 3.6 3.6 0 0 1 -2.371-5.062l3.689-8.47v-.007a1.737 1.737 0 0 0 .106-.332 1.754 1.754 0 0 0 .037-.357v-23.443a6.747 6.747 0 0 1 6.74-6.74h3.646a7.375 7.375 0 0 0 .493 1.466 7.681 7.681 0 0 0 4.448 4.082 7.811 7.811 0 0 0 6.027-.349l10.544-5.2h20.881c.1 0 .209 0 .4.009a20.156 20.156 0 0 1 13.461 5.512 1.749 1.749 0 0 0 1.2.479h22.109a9.249 9.249 0 0 1 9.065 7.49zm43.392 18.856-9.747 9.661a1.749 1.749 0 0 0 -.519 1.243v19.25a4.261 4.261 0 0 1 -4.387 4.251 4.416 4.416 0 0 1 -4.113-4.5v-21.5a4.242 4.242 0 0 1 1.266-3.013 1.806 1.806 0 0 0 .169-.192l4.254-5.617h13.162zm8.877 34.376a4.233 4.233 0 0 1 -4.745-3.783l-2.864-17.666a4.44 4.44 0 0 1 .048-1.338l1.917-9.862 2.167-2.143h6.917l-2.477 12.705a1.73 1.73 0 0 0 -.009.618l2.754 16.771a4.254 4.254 0 0 1 -3.704 4.698zm28.357.028h-8.5v-34.82h8.5zm8.75-38.32h-87.626v-11.44h87.63z"/></svg><br><?php esc_html_e( 'МАСАЖ', 'kmcentre' ); ?></a>
							</div>
						</div>
						<div class= "slider-nav__item">
							<div class="slider-nav__item_img">
								<a class="active" data-slide-index="2"><svg height="75" viewBox="0 0 128 128" width="75" xmlns="http://www.w3.org/2000/svg"><g id="Layer_3" data-name="Layer 3"><path d="m82.373 57.42c0 14.859 6.443 26.5 14.67 26.5s14.67-11.641 14.67-26.5-6.443-26.5-14.67-26.5-14.67 11.641-14.67 26.5zm25.84 0c0 12.468-5.115 23-11.17 23s-11.17-10.532-11.17-23 5.115-23 11.17-23 11.17 10.532 11.17 23z"/><path d="m126.24-.04h-58.39a1.752 1.752 0 0 0 -1.75 1.75v8.7c0 .03.01.06.01.09a.215.215 0 0 0 0 .08l5.13 51.92h-2.06v-1.69a8.766 8.766 0 0 0 -7.06-8.59 33.853 33.853 0 0 0 -6.42-18.34l-4.33-5.97a1.738 1.738 0 0 0 -1.39-.72 1.648 1.648 0 0 0 -1.41.68l-5.2 6.79a17.885 17.885 0 0 1 -14.11 6.96 12.2 12.2 0 0 0 -12.08 10.59 8.758 8.758 0 0 0 -7.13 8.6v1.69h-2.35a1.752 1.752 0 0 0 -1.75 1.75v11.52h-4.21a1.746 1.746 0 0 0 -1.75 1.75v19.73a49.09 49.09 0 0 0 10.37 30.11 1.738 1.738 0 0 0 1.38.68h55.75a1.738 1.738 0 0 0 1.38-.68 48.914 48.914 0 0 0 6.53-11.17v6.02a5.757 5.757 0 0 0 5.75 5.75h31.61a5.751 5.751 0 0 0 5.75-5.75v-17.03a1.625 1.625 0 0 0 .17-.58l9.3-94.02v-.08c0-.03.01-.06.01-.09v-8.7a1.752 1.752 0 0 0 -1.75-1.75zm-112.69 60.85a5.266 5.266 0 0 1 5.27-5.25h14.83a1.75 1.75 0 0 0 0-3.5h-12.9a8.7 8.7 0 0 1 8.51-6.94 21.39 21.39 0 0 0 16.88-8.33l3.78-4.92 2.94 4.07a30.2 30.2 0 0 1 5.73 16.12h-3.38a1.75 1.75 0 0 0 0 3.5h4.86a.984.984 0 0 0 .17-.02.874.874 0 0 0 .16.02h.03a5.262 5.262 0 0 1 5.25 5.25v1.69h-52.13zm-4.1 11.82h21.29a1.75 1.75 0 0 0 0-3.5h-21.29v-3.13h60.33v3.13h-21.29a1.75 1.75 0 1 0 0 3.5h21.29v3.14h-60.33zm66.29 24.62c0 .91-.04 1.84-.1 2.77-.01.13-.01.27-.02.4-.07.91-.16 1.82-.28 2.73a45.626 45.626 0 0 1 -8.72 21.39h-54.02a45.656 45.656 0 0 1 -9.11-27.29v-17.98h72.25zm39.27 24.96a2.253 2.253 0 0 1 -2.25 2.25h-31.61a2.253 2.253 0 0 1 -2.25-2.25v-9.16h36.11zm0-12.66h-36.11v-3.37h36.11zm.34-6.87h-36.42c.01-.12.02-.23.03-.35.07-.67.13-1.34.17-2.01.01-.2.03-.4.04-.6.04-.82.07-1.65.07-2.47v-19.73a1.746 1.746 0 0 0 -1.75-1.75h-1.42l-6.29-63.61h54.53zm9.14-94.02h-54.89v-5.2h54.89z"/><path d="m55.2 95.428a1.75 1.75 0 0 0 -1.75-1.75 15.437 15.437 0 0 0 -7.334 1.831 12.88 12.88 0 0 0 -5.626-6.709 1.748 1.748 0 0 0 -1.754 0 12.876 12.876 0 0 0 -5.623 6.71 15.437 15.437 0 0 0 -7.334-1.831 1.75 1.75 0 0 0 -1.75 1.75v.234a15.494 15.494 0 0 0 3.377 9.658 9.723 9.723 0 0 0 -3.142 3.3 1.751 1.751 0 0 0 0 1.753 9.725 9.725 0 0 0 15.349 1.971 9.724 9.724 0 0 0 15.348-1.972 1.749 1.749 0 0 0 0-1.752 9.723 9.723 0 0 0 -3.141-3.3 15.5 15.5 0 0 0 3.38-9.659zm-8.319 3.693a11.911 11.911 0 0 1 4.706-1.8 12.2 12.2 0 0 1 -6.41 9.064c.189-.1.346-.7.43-.881a15.224 15.224 0 0 0 .68-1.706 12.831 12.831 0 0 0 .59-4.677zm-7.268-6.721a9.322 9.322 0 0 1 0 14.989 9.321 9.321 0 0 1 0-14.989zm-7.263 6.721a12.711 12.711 0 0 0 .577 4.645 12.421 12.421 0 0 0 .742 1.82 8.045 8.045 0 0 1 .372.79 12.2 12.2 0 0 1 -6.4-9.057 11.921 11.921 0 0 1 4.709 1.802zm.347 12.618a6.262 6.262 0 0 1 -4.789-2.245 6.226 6.226 0 0 1 2.045-1.6 15.785 15.785 0 0 0 6.007 2.923 6.235 6.235 0 0 1 -3.26.922zm13.833 0a6.226 6.226 0 0 1 -3.263-.927 15.781 15.781 0 0 0 6.007-2.923 6.216 6.216 0 0 1 2.045 1.6 6.259 6.259 0 0 1 -4.79 2.25z"/><path d="m48.75 52.06h-3.512a1.75 1.75 0 0 0 0 3.5h3.512a1.75 1.75 0 0 0 0-3.5z"/></g></svg><br><?php esc_html_e( 'КОСМЕТОЛОГІЯ', 'kmcentre' ); ?></a>
							</div>
						</div>
						<div class= "slider-nav__item">
							<div class="slider-nav__item_img">
								<a class="active" data-slide-index="3"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="75" height="75" viewBox="0 0 60 60" version="1.1"><g id="007---Autism"><path d="M2.472,37.391 C2.80078202,37.6194083 3.14797678,37.8200989 3.51,37.991 C3.942,38.212 4.667,38.582 4.718,38.841 C4.78085126,39.1828692 4.73974795,39.5357307 4.6,39.854 C4.451,40.174 4.295,40.449 4.16,40.686 C3.794,41.333 3.181,42.417 4.105,43.172 C4.321,43.372 4.694,43.661 5.034,43.922 C4.78370505,44.359639 4.67048978,44.8623148 4.709,45.365 C4.76206133,45.7609003 4.97674391,46.1171358 5.302,46.349 C5.524,46.505 5.761,46.641 5.994,46.774 C6.586,47.11 6.817,47.267 6.894,47.581 C6.94956021,47.9834802 6.91541128,48.3932674 6.794,48.781 C6.735,49.048 6.678,49.316 6.644,49.581 C6.54469822,50.4923539 6.65729272,51.4143284 6.973,52.275 C7.42281456,53.8638526 8.85996076,54.9707558 10.511,55 C10.7865929,54.9991845 11.0613714,54.9700312 11.331,54.913 C14.1877778,54.2358183 17.0028445,53.3934018 19.762,52.39 C22.015,53.712 23.438,57.732 23.809,59.241 C23.9197719,59.6870872 24.3203652,60.0002183 24.78,60 L49.88,59.93 C50.198095,59.9293833 50.4969218,59.7774667 50.6848608,59.5208273 C50.8727998,59.2641879 50.9274293,58.9334437 50.832,58.63 C50.584758,57.9524505 50.2437748,57.3128965 49.819,56.73 C49.5792082,56.3843183 49.3630492,56.0228282 49.172,55.648 C48.7190564,54.7315437 48.3414759,53.7797332 48.043,52.802 C47.4719279,50.8701776 47.0759954,48.8908497 46.86,46.888 C46.5989687,44.5717463 46.8202794,42.2265341 47.51,40 L50,40 C51.6568542,40 53,38.6568542 53,37 L53,24 C53,23.4477153 52.5522847,23 52,23 L47.964,23 C47.987498,22.834361 47.9995267,22.6672968 48,22.5 C48,20.5670034 46.4329966,19 44.5,19 C42.5670034,19 41,20.5670034 41,22.5 C41.0004733,22.6672968 41.012502,22.834361 41.036,23 L38,23 L38,19.965 C38.5525556,20.0436927 39.1158892,19.9905804 39.644,19.81 C40.7855962,19.4211999 41.6437712,18.4680086 41.911,17.292 C42.1726918,16.1657743 41.8612892,14.9829783 41.0789461,14.1316227 C40.2966031,13.2802671 39.1442834,12.8702217 38,13.036 L38,9 C38,8.44771525 37.5522847,8 37,8 L24,8 C22.3431458,8 21,9.34314575 21,11 L21,37 C21,38.6568542 22.3431458,40 24,40 L45.41,40 C44.7806242,42.3211618 44.5997257,44.741062 44.877,47.13 C45.1076596,49.2534653 45.5286863,51.3519052 46.135,53.4 C46.4640482,54.4749292 46.879054,55.5216399 47.376,56.53 C47.6041562,56.9828684 47.8630969,57.4195592 48.151,57.837 L48.218,57.937 L25.536,58 C24.8721602,55.6278229 23.6672676,53.4419012 22.016,51.614 L22.916,51.299 C23.586,51.065 24.091,50.889 24.35,50.806 C28.634,49.451 30.776,47.5 30.718,45.006 C30.7050213,44.4537153 30.2467847,44.0165213 29.6945,44.0295 C29.1422153,44.0424787 28.7050213,44.5007153 28.718,45.053 C28.742,46.082 27.899,47.583 23.742,48.898 C23.471,48.985 22.949,49.167 22.256,49.409 C21.513,49.669 20.568,49.999 19.526,50.353 L19.519,50.353 C16.7047997,51.3845725 13.8333482,52.2526858 10.919,52.953 C9.98843633,53.1296801 9.08382639,52.5420363 8.867,51.62 C8.65336419,51.0511056 8.5729723,50.4408114 8.632,49.836 C8.658,49.626 8.706,49.418 8.752,49.209 C8.95576498,48.5176489 8.98564963,47.7866772 8.839,47.081 C8.54814599,46.1623751 7.87220583,45.4156291 6.987,45.035 L6.78,44.917 C6.88797779,44.6989963 7.01691908,44.4920204 7.165,44.299 C7.33275906,44.0864363 7.40714209,43.8149259 7.3711438,43.5465408 C7.33514552,43.2781557 7.19184383,43.0358431 6.974,42.875 C6.734,42.699 6.168,42.275 5.754,41.941 C5.802,41.854 5.854,41.758 5.904,41.671 C6.061,41.396 6.24,41.076 6.415,40.706 C6.73144627,40.0050454 6.82465982,39.2237273 6.682,38.468 C6.442,37.248 5.273,36.65 4.421,36.214 C4.13330073,36.0772824 3.85736569,35.9170729 3.596,35.735 C3.38336862,35.5912941 3.18391862,35.42899 3,35.25 C3,35.098 3.163,34.73 3.925,33.803 C5.71607044,31.4972449 7.2517206,29.0039928 8.505,26.367 C8.73934083,25.9538574 8.87135428,25.4906102 8.89,25.016 C8.78670832,24.0108401 8.25404392,23.0993679 7.429,22.516 C7.029,22.145 6.644,21.795 6.599,21.469 C6.60563423,21.2329751 6.65864086,21.0005614 6.755,20.785 C6.918,20.356 7.106,19.936 7.295,19.515 C7.70505054,18.6694994 8.03256792,17.786408 8.273,16.878 L8.373,16.452 C8.58408599,15.4522697 8.85626318,14.4664205 9.188,13.5 C9.97629737,11.324256 11.1967986,9.33056659 12.776,7.639 C14.689,5.615 17.7,4.183 21.74,3.38 C24.8292288,2.70168567 27.9639186,2.25014863 31.119,2.029 C35.6754767,1.8013892 40.1863531,3.02812624 44,5.532 L44,6.5 C44,6.87877206 44.2140024,7.22503518 44.5527864,7.39442719 C44.8915704,7.56381921 45.2969824,7.52726324 45.6,7.3 C46.2534706,6.8389544 47.1509688,6.95401828 47.667,7.565 C47.9576482,7.92833557 48.0642988,8.4054953 47.956,8.858 C47.8386132,9.35040069 47.4781778,9.74891548 47,9.915 C46.5248129,10.0823723 45.9974538,10.0017306 45.594,9.7 C45.291021,9.47628091 44.8880413,9.44176584 44.5514313,9.61070436 C44.2148213,9.77964288 44.0016789,10.1233786 44,10.5 L44,15 C44,16.1045695 44.8954305,17 46,17 L50.5,17 C50.8787721,17 51.2250352,16.7859976 51.3944272,16.4472136 C51.5638192,16.1084296 51.5272632,15.7030176 51.3,15.4 C50.8358758,14.7811677 50.9184499,13.9108787 51.4906755,13.3903766 C52.0629012,12.8698745 52.9370988,12.8698745 53.5093245,13.3903766 C54.0815501,13.9108787 54.1641242,14.7811677 53.7,15.4 C53.4727368,15.7030176 53.4361808,16.1084296 53.6055728,16.4472136 C53.7749648,16.7859976 54.1212279,17 54.5,17 L60,17 C60.5522847,17 61,16.5522847 61,16 L61,3 C61,1.34314575 59.6568542,1.01453063e-16 58,0 L45,0 C44.4477153,-3.38176876e-17 44,0.44771525 44,1 L44,3.137 C40.0598774,0.878619432 35.5536043,-0.199526018 31.018,0.031 C27.7640828,0.25668862 24.5310865,0.720598351 21.345,1.419 C16.836,2.319 13.559,3.9 11.322,6.264 C9.55741114,8.15217091 8.19304573,10.3778111 7.311,12.807 C6.95086895,13.8522819 6.65498362,14.9186045 6.425,16 L6.325,16.422 C6.1115707,17.2063052 5.82532664,17.9689531 5.47,18.7 C5.27,19.156 5.062,19.612 4.883,20.084 C4.65284498,20.6094053 4.56192334,21.1852424 4.619,21.756 C4.82021123,22.6521932 5.3374331,23.4459531 6.076,23.992 C6.565,24.44 6.886,24.756 6.89,25.031 C6.85421996,25.1830023 6.79697882,25.3291355 6.72,25.465 C5.52247783,27.986227 4.05560327,30.370443 2.345,32.576 C1.62,33.458 0.86,34.487 1.02,35.517 C1.23211923,36.311598 1.75555427,36.9871609 2.472,37.391 Z M23,11 C23,10.4477153 23.4477153,10 24,10 L36,10 L36,14.5 C36,14.8787721 36.2140024,15.2250352 36.5527864,15.3944272 C36.8915704,15.5638192 37.2969824,15.5272632 37.6,15.3 C38.2534706,14.8389544 39.1509688,14.9540183 39.667,15.565 C39.9576482,15.9283356 40.0642988,16.4054953 39.956,16.858 C39.8386132,17.3504007 39.4781778,17.7489155 39,17.915 C38.523272,18.082319 37.9944518,18.0001248 37.591,17.696 C37.2874703,17.4736199 36.8848026,17.4404479 36.5489723,17.6101572 C36.213142,17.7798665 36.0010043,18.1237259 36,18.5 L36,23 L32.966,23 C33.0434465,22.4480056 32.9900084,21.8855346 32.81,21.358 C32.4220813,20.2156125 31.4692462,19.3562817 30.293,19.088 C29.2525622,18.8459721 28.1585566,19.0923336 27.3222947,19.7569797 C26.4860328,20.4216257 25.9990785,21.431783 26,22.5 C26.0004733,22.6672968 26.012502,22.834361 26.036,23 L23,23 L23,11 Z M23,37 L23,25 L27.5,25 C27.8787721,25 28.2250352,24.7859976 28.3944272,24.4472136 C28.5638192,24.1084296 28.5272632,23.7030176 28.3,23.4 C27.8165311,22.7519094 27.933739,21.8372456 28.565,21.332 C28.9288145,21.0415321 29.4063339,20.9352526 29.859,21.044 C30.3515615,21.1606604 30.7500728,21.5214339 30.915,22 C31.0835295,22.4764276 31.000832,23.0056914 30.695,23.408 C30.4719286,23.711685 30.4385041,24.1149806 30.6085375,24.4512455 C30.778571,24.7875103 31.1231906,24.9996467 31.5,25 L36,25 L36,28.036 C34.6432091,27.8398828 33.2968315,28.4539384 32.5553829,29.6070201 C31.8139344,30.7601018 31.8139344,32.2398982 32.5553829,33.3929799 C33.2968315,34.5460616 34.6432091,35.1601172 36,34.964 L36,38 L24,38 C23.4477153,38 23,37.5522847 23,37 Z M37.445,32.6 C37.1044465,32.4328869 36.6988451,32.4715524 36.396,32.7 C35.7771677,33.1641242 34.9068787,33.0815501 34.3863766,32.5093245 C33.8658745,31.9370988 33.8658745,31.0629012 34.3863766,30.4906755 C34.9068787,29.9184499 35.7771677,29.8358758 36.396,30.3 C36.6990176,30.5272632 37.1044296,30.5638192 37.4432136,30.3944272 C37.7819976,30.2250352 37.996,29.8787721 37.996,29.5 L37.996,25 L42.496,25 C42.8747721,25 43.2210352,24.7859976 43.3904272,24.4472136 C43.5598192,24.1084296 43.5232632,23.7030176 43.296,23.4 C42.8318758,22.7811677 42.9144499,21.9108787 43.4866755,21.3903766 C44.0589012,20.8698745 44.9330988,20.8698745 45.5053245,21.3903766 C46.0775501,21.9108787 46.1601242,22.7811677 45.696,23.4 C45.4687368,23.7030176 45.4321808,24.1084296 45.6015728,24.4472136 C45.7709648,24.7859976 46.1172279,25 46.496,25 L51,25 L51,37 C51,37.5522847 50.5522847,38 50,38 L38,38 L38,33.5 C38.0017022,33.1187272 37.7864467,32.7696643 37.445,32.6 Z M46,2 L58,2 C58.5522847,2 59,2.44771525 59,3 L59,15 L55.964,15 C55.987498,14.834361 55.9995267,14.6672968 56,14.5 C56,12.5670034 54.4329966,11 52.5,11 C50.5670034,11 49,12.5670034 49,14.5 C49.0004733,14.6672968 49.012502,14.834361 49.036,15 L46,15 L46,11.965 C46.5525547,12.0437308 47.1158972,11.9906176 47.644,11.81 C48.7855962,11.4211999 49.6437712,10.4680086 49.911,9.292 C50.1726918,8.16577428 49.8612892,6.98297831 49.0789461,6.13162268 C48.2966031,5.28026705 47.1442834,4.87022168 46,5.036 L46,2 Z" id="Shape"/><path d="M12.166,17.791 L12.266,17.357 C12.4434665,16.5211797 12.6684992,15.6961711 12.94,14.886 C13.5415702,13.2145911 14.4776381,11.6834003 15.691,10.386 C16.4691984,9.60340685 17.3932981,8.98099852 18.411,8.554 C18.7482795,8.41522295 18.9832579,8.10382979 19.0241772,7.74141821 C19.0650965,7.37900663 18.9054665,7.02305888 18.6076219,6.81257101 C18.3097773,6.60208314 17.9209594,6.57044073 17.593,6.73 C16.3345802,7.26296453 15.1937669,8.03913896 14.236,9.014 C12.8357424,10.5085629 11.755349,12.2732621 11.061,14.2 C10.7592412,15.0882941 10.5098134,15.9935133 10.314,16.911 L10.214,17.351 C10.1542938,17.6102999 10.2003915,17.8827032 10.3420905,18.1079201 C10.4837895,18.3331371 10.7094041,18.4925943 10.969,18.551 C11.041488,18.5676417 11.1156262,18.5760283 11.19,18.576 C11.6592092,18.5757119 12.0651201,18.2492364 12.166,17.791 Z" id="Shape"/><circle id="Oval" cx="10" cy="21" r="1"/></g></g></svg><br><?php esc_html_e( 'ПСИХОЛОГІЯ', 'kmcentre' ) ?></a>
							</div>
						</div>
						<div class= "slider-nav__item">
							<div class="slider-nav__item_img">
								<a class="active" data-slide-index="4"><svg height="75" viewBox="0 -44 479.97011 479" width="75" xmlns="http://www.w3.org/2000/svg"><path d="m383.984375 104.472656h16v32h-16zm0 0"/><path d="m351.984375 104.472656h16v32h-16zm0 0"/><path d="m319.984375 104.472656h16v32h-16zm0 0"/><path d="m287.984375 104.472656h16v32h-16zm0 0"/><path d="m383.984375 256.472656h16v16h-16zm0 0"/><path d="m351.984375 256.472656h16v16h-16zm0 0"/><path d="m319.984375 256.472656h16v16h-16zm0 0"/><path d="m287.984375 256.472656h16v16h-16zm0 0"/><path d="m23.984375 231.871094v48.601562c.027344 22.082032 17.921875 39.972656 40 40h144.625l142.398437 68.335938c8.921876 4.273437 19.175782 4.820312 28.5 1.515625 9.324219-3.300781 16.949219-10.179688 21.1875-19.117188l24.738282-52c17.882812-4.34375 30.492187-20.332031 30.550781-38.734375v-25.472656l20.359375-42.765625c8.839844-18.601563 1.007812-40.847656-17.535156-49.808594l-2.824219-1.351562v-56.601563c-.027344-22.082031-17.917969-39.972656-40-40h-161.289063l-125.71875-60.335937c-8.917968-4.273438-19.175781-4.820313-28.496093-1.515625-9.324219 3.300781-16.949219 10.179687-21.191407 19.117187l-20.59375 43.269531c-19.816406 2.613282-34.648437 19.476563-34.710937 39.464844v33.464844l-20.359375 42.773438c-8.835938 18.601562-1.003906 40.847656 17.535156 49.808593zm432-52.382813c7.714844 6.21875 10.15625 16.914063 5.90625 25.863281l-5.90625 12.410157zm-69.757813 184.800781c-2.410156 5.101563-6.75 9.03125-12.0625 10.917969-5.316406 1.890625-11.164062 1.582031-16.25-.855469l-112.328124-53.878906h161.519531zm53.757813-83.816406c0 13.253906-10.746094 24-24 24h-264v-92.511718c17.792969 17.117187 41.535156 26.664062 66.226563 26.632812v-16c-20.539063.023438-40.292969-7.902344-55.113282-22.121094h276.886719zm-280.542969-100.96875 15.742188-39.558594c3.117187-7.664062 11.011718-12.261718 19.214844-11.183593 8.203124 1.074219 14.644531 7.550781 15.675781 15.761719 1.03125 8.207031-3.609375 16.078124-11.289063 19.148437zm256.542969-99.03125c13.253906 0 24 10.746094 24 24v80h-249.984375l14.753906-5.921875c14.515625-5.753906 23.296875-20.601562 21.351563-36.09375-1.945313-15.492187-14.128907-27.707031-29.613281-29.695312-15.488282-1.988281-30.359376 6.753906-36.15625 21.253906l-8.351563 20.96875v-74.511719zm-322.238281-51.847656c2.40625-5.101562 6.746094-9.03125 12.0625-10.921875 5.3125-1.886719 11.160156-1.578125 16.25.859375l95.671875 45.910156h-141.050781zm-53.761719 75.847656c0-13.253906 10.746094-24 24-24h72v68.929688l-6.128906-15.386719c-5.824219-14.425781-20.660157-23.097656-36.089844-21.09375-15.429687 2-27.5625 14.164063-29.515625 29.601563-1.957031 15.433593 6.757812 30.242187 21.199219 36.027343l14.742187 5.921875h-60.207031zm90.777344 75.03125-39.34375-15.832031c-7.753907-3.035156-12.46875-10.9375-11.457031-19.203125 1.011718-8.265625 7.496093-14.792969 15.753906-15.867188 8.257812-1.074218 16.191406 3.578126 19.285156 11.3125zm-90.777344 20.96875h87.113281c-14.824218 14.21875-34.574218 22.144532-55.113281 22.121094v16c23.648437.023438 46.457031-8.746094 64-24.601562v90.480468h-72c-13.253906 0-24-10.746094-24-24zm-21.902344-12.878906 5.902344-12.417969v38.28125c-7.714844-6.21875-10.15625-16.914062-5.902344-25.863281zm0 0"/></svg><br><?php esc_html_e( 'ПОДАРУНКОВІ СЕРТИФІКАТИ', 'kmcentre' ); ?></a>
							</div>
						</div>
						<div class= "slider-nav__item">
							<div class="slider-nav__item_img">
								<a class="active" data-slide-index="5"><svg xmlns="http://www.w3.org/2000/svg" id="outline" viewBox="0 0 512 512" width="75" height="75"><path d="M480.80225,300.78149l7.04248-35.21264A8.00053,8.00053,0,0,0,480,256h-.69458l-7.33862-80.72412a8.00609,8.00609,0,0,0-1.02051-3.24512l-27.19336-47.5874a39.87247,39.87247,0,0,0-16.84082-15.93164L396.98193,93.54663A31.79971,31.79971,0,0,0,400,80V40a7.99943,7.99943,0,0,0-3.5625-6.65625L377.0791,20.43848A26.42157,26.42157,0,0,0,336,42.42236V80a31.79971,31.79971,0,0,0,3.01807,13.54663l-28.11182,14.05591a40.05243,40.05243,0,0,0-18.877,20.02l-18.12207,42.28467L264,174.15332V168a8.00008,8.00008,0,0,0-8-8H176a8.00008,8.00008,0,0,0-8,8v48a8.00008,8.00008,0,0,0,8,8H296a8.00091,8.00091,0,0,0,6.86035-3.88428l12.82031-21.36816L320,224.66455V272H216V256a7.99967,7.99967,0,0,0-5.46973-7.58936L166.0498,233.584l-10.61816-26.54687A39.75054,39.75054,0,0,0,151.75488,200H152V128a56,56,0,0,0-112,0v63.76013a40.2566,40.2566,0,0,0-11.92578,14.20276L16.84473,228.42236A7.99737,7.99737,0,0,0,16,232v24a31.80409,31.80409,0,0,0,4.72119,16.70813A7.996,7.996,0,0,0,16,280V488a8.00008,8.00008,0,0,0,8,8H464a32.03635,32.03635,0,0,0,32-32V328A32.0055,32.0055,0,0,0,480.80225,300.78149ZM416,432V376h24v56Zm24,16v32H416V448ZM288,344H264V320h24Zm16-16h16v16H304Zm32,16V232h64v24H376a8.00053,8.00053,0,0,0-7.84473,9.56885l7.04248,35.21264A32.0055,32.0055,0,0,0,360,328V480H344V352A8.00008,8.00008,0,0,0,336,344Zm144-16v5.333L460,360h-4V312h8A16.01833,16.01833,0,0,1,480,328Zm-94.24121-56h15.68262l4.8,24H392c-.481,0-.95825.01538-1.43408.03638ZM392,312h8v48h-4l-20-26.667V328A16.01833,16.01833,0,0,1,392,312Zm24,0h24v48H416Zm23.80273-47.80591L433.44141,296H422.55859L416,263.20752v-38.543L420.56543,197.27l4.00977,6.01465ZM376,368v-7.99976l9.59961,12.79956A7.99952,7.99952,0,0,0,392,376h8v56H376Zm80,8h8a7.99952,7.99952,0,0,0,6.40039-3.2002L480,360.00024V432H456Zm8-80H449.75854l4.8-24h15.68262l-4.80713,24.03638C464.95825,296.01538,464.481,296,464,296Zm-9.75391-40-13.197-52.787,16.28174-12.2113L463.24023,256ZM352,80V64h32V80a16,16,0,0,1-32,0Zm10.42188-48a10.38659,10.38659,0,0,1,5.78124,1.75049L384,44.28174V48H352V42.42236A10.43354,10.43354,0,0,1,362.42188,32ZM184,176h24v32H184Zm64,32H224V176h24Zm16,0V191.56073l13.37207-5.7309,15.22266,20.29676L291.4707,208Zm49.13965-36.11572-11.656,19.427-12.30835-16.411,17.561-40.975a24.02573,24.02573,0,0,1,11.32617-12.0122l31.52734-15.76368a31.89109,31.89109,0,0,0,36.82032,0l33.3457,16.67286a23.9154,23.9154,0,0,1,10.10449,9.55908l23.68994,41.4563-19.585,14.68853L422.65625,171.5625a8.00023,8.00023,0,0,0-14.54785,3.12256L401.22266,216H334.77734l-6.88574-41.31494a8.00059,8.00059,0,0,0-14.75195-2.80078ZM320,312H304a8.00008,8.00008,0,0,0-8-8H256a8.00008,8.00008,0,0,0-8,8H32V288H320ZM112,262.24609V272H96V258.24609ZM200,272H184V256.43286l16,5.33325ZM96,88a40.04552,40.04552,0,0,1,40,40v57.87048l-17.91357-10.7481A31.90253,31.90253,0,0,0,128,152V128a8.00035,8.00035,0,0,0-2.34277-5.65674l-16-16A8.00063,8.00063,0,0,0,97.34375,107.562l-7.125,10.6875A21.8521,21.8521,0,0,1,72,128a8.00008,8.00008,0,0,0-8,8v16a31.89149,31.89149,0,0,0,9.22,22.44568l-17.22,8.61V128A40.04552,40.04552,0,0,1,96,88Zm16,64a16,16,0,0,1-32,0v-8.84863a37.815,37.815,0,0,0,23.53125-16.02686l1.71191-2.56738L112,131.314ZM89.88867,184h11.89551l14.31348,8.58826L96,212.686,74.83887,191.5249ZM32,233.88867l10.38477-20.77a24.152,24.152,0,0,1,10.73437-10.73388l6.63452-3.3172,30.58911,30.58917a8.00063,8.00063,0,0,0,11.31446,0l28.583-28.583.39942.23969a24.1413,24.1413,0,0,1,9.93652,11.6665l11.9961,29.99121a7.99992,7.99992,0,0,0,4.89746,4.61817L168,251.09943V272H128V256a8.00005,8.00005,0,0,0-6.05957-7.76123l-64-16-3.88086,15.52246L80,254.24622V272H48a16.01833,16.01833,0,0,1-16-16ZM208,352V480H32V328H248v16H216A8.00008,8.00008,0,0,0,208,352ZM328,480H224V360h24v16h16V360h24v16h16V360h24Zm64,0a16.01833,16.01833,0,0,1-16-16V448h24v32Zm72,0h-8V448h24v16A16.01833,16.01833,0,0,1,464,480Z"/></svg><?php esc_html_e( 'МЕДИЧНИЙ ТУРИЗМ', 'kmcentre' ); ?></a>
							</div>
						</div>
			</div>	
			</div>
		</div>
	</div>
</div>
<!-- /services -->
<div class="site-main__container">
	<div class="site-main__container _container">
<!-- slider-services -->
		<div class="slider-services">
			<div class="slider-services__block">
				<div class="slider-services__image">
						<div class="slider-services__image-one">
							<img src="<?php echo get_template_directory_uri(); ?>/images/services/stomat-1.jpg" alt="">
						</div>
						<div class="slider-services__image-two">
							<img src="<?php echo get_template_directory_uri(); ?>/images/services/stomat-2.jpg" alt="">
						</div>
				</div>
				<div class="slider-services__text-block"><h4><?php esc_html_e( 'СТОМАТОЛОГІЯ', 'kmcentre' ); ?></h4><p><?php esc_html_e( 'Ми дбаємо про здоров’я і красивої усмішці наших пацієнтів, 
застосовуючи найкращі досягнення сучасної стоматології 
для ефективного і безпечного лікування зубів.', 'kmcentre' ); ?></p></div>
				<div class="slider-services__button-block"><a class="btn rounded" href="#"><span><?php esc_html_e( 'Дізнатися більше', 'kmcentre' ); ?></span></a></div>
			</div>
			<div class="slider-services__block">
				<div class="slider-services__image">
						<div class="slider-services__image-one">
							<img src="<?php echo get_template_directory_uri(); ?>/images/services/massage-1.jpg" alt="">
						</div><!-- /.slider-services__image-one -->
						<div class="slider-services__image-two">
							<img src="<?php echo get_template_directory_uri(); ?>/images/services/massage-2.jpg" alt="">
						</div><!-- /.slider-services__image-two -->
						
				</div>
				<div class="slider-services__text-block"><h4><?php esc_html_e( 'МАСАЖ', 'kmcentre' ); ?></h4><p><?php esc_html_e( 'Ми дбаємо про здоров’я і красивої усмішці наших пацієнтів, 
застосовуючи найкращі досягнення сучасної стоматології 
для ефективного і безпечного лікування зубів.', 'kmcentre' ); ?></p></div>
				<div class="slider-services__button-block"><a class="btn rounded" href="#"><span><?php esc_html_e( 'Дізнатися більше', 'kmcentre' ); ?></span></a></div>
			</div>
			<div class="slider-services__block">
				<div class="slider-services__image">
						<div class="slider-services__image-one">
							<img src="<?php echo get_template_directory_uri(); ?>/images/services/kosmetolog-1.jpg" alt="">
						</div><!-- /.slider-services__image-one -->
						<div class="slider-services__image-two">
							<img src="<?php echo get_template_directory_uri(); ?>/images/services/kosmetolog-2.jpg" alt="">
						</div><!-- /.slider-services__image-two -->
						
				</div>
				<div class="slider-services__text-block"><h4><?php esc_html_e( 'КОСМЕТОЛОГІЯ', 'kmcentre' ); ?></h4><p><?php esc_html_e( 'Ми дбаємо про здоров’я і красивої усмішці наших пацієнтів, 
застосовуючи найкращі досягнення сучасної стоматології 
для ефективного і безпечного лікування зубів.', 'kmcentre' ); ?></p></div>
				<div class="slider-services__button-block"><a class="btn rounded" href="#"><span><?php esc_html_e( 'Дізнатися більше', 'kmcentre' ); ?></span></a></div>
			</div>
			<div class="slider-services__block">
				<div class="slider-services__image">
						<div class="slider-services__image-one">
							<img src="<?php echo get_template_directory_uri(); ?>/images/services/psiho-1.jpg" alt="">
						</div><!-- /.slider-services__image-one -->
						<div class="slider-services__image-two">
							<img src="<?php echo get_template_directory_uri(); ?>/images/services/psiho-2.jpg" alt="">
						</div><!-- /.slider-services__image-two -->
						
				</div>
				<div class="slider-services__text-block"><h4><?php esc_html_e( 'ПСИХОЛОГІЯ', 'kmcentre' ); ?></h4><p><?php esc_html_e( 'Ми дбаємо про здоров’я і красивої усмішці наших пацієнтів, 
застосовуючи найкращі досягнення сучасної стоматології 
для ефективного і безпечного лікування зубів.', 'kmcentre' ); ?></p></div>
				<div class="slider-services__button-block"><a class="btn rounded" href="#"><span><?php esc_html_e( 'Дізнатися більше', 'kmcentre' ); ?></span></a></div>
			</div>
			<div class="slider-services__block">
				<div class="slider-services__image">
						<div class="slider-services__image-one">
							<img src="<?php echo get_template_directory_uri(); ?>/images/services/sert-1.jpg" alt="">
						</div><!-- /.slider-services__image-one -->
						<div class="slider-services__image-two">
							<img src="<?php echo get_template_directory_uri(); ?>/images/services/sert-2.jpg" alt="">
						</div><!-- /.slider-services__image-two -->
						
				</div>
				<div class="slider-services__text-block"><h4><?php esc_html_e( 'ПОДАРУНКОВІ СЕРТИФІКАТИ', 'kmcentre' ); ?></h4><p><?php esc_html_e( 'Ми дбаємо про здоров’я і красивої усмішці наших пацієнтів, 
застосовуючи найкращі досягнення сучасної стоматології 
для ефективного і безпечного лікування зубів.', 'kmcentre' ); ?></p></div>
				<div class="slider-services__button-block"><a class="btn rounded" href="#"><span><?php esc_html_e( 'Дізнатися більше', 'kmcentre' ); ?></span></a></div>
			</div>
			<div class="slider-services__block">
				<div class="slider-services__image">
						<div class="slider-services__image-one">
							<img src="<?php echo get_template_directory_uri(); ?>/images/services/medtur-1.jpg" alt="">
						</div><!-- /.slider-services__image-one -->
						<div class="slider-services__image-two">
							<img src="<?php echo get_template_directory_uri(); ?>/images/services/medtur-2.jpg" alt="">
						</div><!-- /.slider-services__image-two -->
						
				</div>
				<div class="slider-services__text-block"><h4><?php esc_html_e( 'МЕДИЧНИЙ ТУРИЗМ', 'kmcentre' ); ?></h4><p><?php esc_html_e( 'Ми дбаємо про здоров’я і красивої усмішці наших пацієнтів, 
застосовуючи найкращі досягнення сучасної стоматології 
для ефективного і безпечного лікування зубів.', 'kmcentre' ); ?></p></div>
				<div class="slider-services__button-block"><a class="btn rounded" href="#"><span><?php esc_html_e( 'Дізнатися більше', 'kmcentre' ); ?></span></a></div>
			</div>
		</div>
<!-- /slider-services -->
	</div>
	<div class="site-main__container _container">
		<div class="site-main__decor block-decor">
			<div class="block-decor__items">
			<span></span>
			</div>
		</div>
	</div>
	<div class="site-main__container _container">
	<div class="about__container ">
		<div class="about__title"><h1><?php esc_html_e( 'Про центр', 'kmcentre' ); ?></h1>
		</div>
		<div class="about__block">
			<div class="about__block_row">
				<div class="test">
				<p>KRAVCHENKO MEDIKAL CENTRE  піклується сьогодні про здоров’я Ваших зубів, красу обличчя і тіла, емоційний і фізичний баланс, забезпечує довгостроковий позитивний результат на засадах обраної більше 20 років тому філософії служіння. І це не високопарні слова, це перевірений часом і нашими з Вами стосунками підхід допомогти кожному клієнту отримати більше, ніж Ви очікуєте, економлячи Ваші час і гроші в прозорих довірливих стосунках. Ми щиро вдячні кожному з Вас за прихильність, довіру свого здоров’я, багаторічну дружбу сім’ями, щиру радість нашим успіхам і досягненням. За віру в спільне завтра, в те, що ми є і будемо ще кращі.</p>
				<p>ЗВІДКИ МИ? Історія KMCENTRE   почалася в далекому 1995 році. Саме тоді на територію України, її столиці Києва прийшла німецька приватна стоматологічна клініка «КЕБОТ». В 1999 році вона була розділена між чотирма власниками на підрозділи. На Артема, нинішня Січових Стрільців, в нашому будинку 5-Б народився Медичний центр «НАДІЯ», в якому почали працювати стоматологічний, косметологічний та масажний кабінети. В 2005 році були змінені власники, та ідея об’єднання різнопланових послуг на одній території залишилась. Змінювались ліцензії, лікарі, спеціалісти, навіть приміщення змінилось. Із орендованого «НАДІЯ» в 2007 перейшла у власне приміщення, чим твердо запевнила своїх клієнтів і пацієнтів про стабільність намірів і стратегії компанії бути з Вами в радості і горі, дбати про Вашу красу і здоров’я все життя. </p>
				</div>
			</div>
			<div class="about__block_row">
				<div class="about__block_row column">
				<div class="item"><img src="<?php echo get_template_directory_uri(); ?>/images/about/about1-1.png" alt=""></div>
				<div class="item"><img src="<?php echo get_template_directory_uri(); ?>/images/about/about1-2.png" alt=""></div>
				</div>
				<div class="about__block_row column">
				<div class="item"><img src="<?php echo get_template_directory_uri(); ?>/images/about/about2-1.png" alt=""></div>
				<div class="item"><img src="<?php echo get_template_directory_uri(); ?>/images/about/about2-2.png" alt=""></div>
				<div class="item"><img src="<?php echo get_template_directory_uri(); ?>/images/about/about2-3.png" alt=""></div>
				</div>
			</div>
		</div>
	</div>
	</div>

	<div class="testimonials__container">
		<div class="_container">
			<div class="testimonials__container _title"><h1><?php esc_html_e( 'Відгуки', 'kmcentre' ); ?></h1></div>
				<div class="testimonials__container _block">
					<div class="testimonials__container _nav">
					<a class="tm-prev" href=""><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 512" width="45" height="65" fill="#fff"><path d="M31.7 239l136-136c9.4-9.4 24.6-9.4 33.9 0l22.6 22.6c9.4 9.4 9.4 24.6 0 33.9L127.9 256l96.4 96.4c9.4 9.4 9.4 24.6 0 33.9L201.7 409c-9.4 9.4-24.6 9.4-33.9 0l-136-136c-9.5-9.4-9.5-24.6-.1-34z"/></svg></a>
					<a class="tm-next" href=""><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 512" width="45" height="65" fill="#fff"><path d="M224.3 273l-136 136c-9.4 9.4-24.6 9.4-33.9 0l-22.6-22.6c-9.4-9.4-9.4-24.6 0-33.9l96.4-96.4-96.4-96.4c-9.4-9.4-9.4-24.6 0-33.9L54.3 103c9.4-9.4 24.6-9.4 33.9 0l136 136c9.5 9.4 9.5 24.6.1 34z"/></svg></a>
					</div>

					<div class="testimonials__slider_box">
					<div class="testimonials__slider">
					<?php
					$args = array(
						'post_type'	=> array( 'testimonials' ),
						'post_status'	=> array( 'publish' ),
						'posts_per_page'	=> 5,
					);

					// The Query
					$query = new WP_Query( $args );

					// The Loop
					if ( $query->have_posts() ) {
						while ( $query->have_posts() ) {
							$query->the_post();
?>							

							<div class="testimonials__container _text">
						<div class="surname"><?php echo  get_the_title(); ?></div>
						<div class="text"><?php echo get_the_excerpt(); ?></div>
						<div class="date"><?php echo   get_the_date(); ?></div>
					</div>
							
							
							<?php
						}

					}
					wp_reset_postdata();
					?>



					</div>
					</div>

				</div>
			</div>
	</div>

	<div class="site-main__container _container">
	<div class="last-post__container">
		<div class="last-post _title"><h1><?php esc_html_e( 'Свіжі новини', 'kmcentre' ); ?></h1></div>
			<div class="last-post__row">
			<?php // WP_Query arguments
				$args = array(
					'post_type'              => array( 'post' ),
					'post_status'            => array( 'publish' ),
					'posts_per_page'         => '3',
				);

				// The Query
				$query = new WP_Query( $args );

				// The Loop
				if ( $query->have_posts() ) {
					while ( $query->have_posts() ) {
						$query->the_post();
						// do something

				?>
				<!-- Start Last Post Item -->
				<div class="last-post__item">
					<div class="last-post__item_block">
						<a href="<?php echo get_permalink(); ?>" class="radius_niz"><img src="<?php the_post_thumbnail_url( 'list-post' ); ?>" alt=""></a>
						<div class="last-post__item_content">
							<a href="<?php echo get_permalink(); ?>" class="last-post__item_content_title"><?php echo  get_the_title(); ?></a>
							<div class="last-post__item_content_date"><?php echo   get_the_date(); ?></div>
							<div class="last-post__item_content_desk"><?php echo get_the_excerpt(); ?></div>
						</div>
					</div>
				</div>
<!-- End Last Post Item -->

				<?php
									}
								}

				// Restore original Post Data
				wp_reset_postdata();
			?>
		
			</div>
		</div>
	</div>

	<div class="site-main__container _container">
		<div class="site-main__decor block-decor">
			<div class="block-decor__items">
			<span></span>
			</div>
		</div>
	</div>

	<div class="site-main__container _container">
	<div class="contact-block__container">
		<div class="contact-block _title"><h1><?php esc_html_e( 'Зв’язатися з нами', 'kmcentre' ); ?></h1></div>
		<div class="contact_map_content">
		<iframe src="https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d1270.1214599539583!2d30.504390000000004!3d50.455201!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0x766665fb792fc659!2z0JrRgNCw0LLRh9C10L3QutC-INCc0LXQtNC40LrQsNC7INCm0LXQvdGC0YAgKEtyYXZjaGVua28gTWVkaWNhbCBDZW50cmUpIERlbnRhbCBjbGluaWMgL9CS0LfRgNC-0YHQu9Cw0Y8g0YHRgtC-0LzQsNGC0L7Qu9C-0LPQuNGP!5e0!3m2!1sru!2sua!4v1589293653877!5m2!1sru!2sua" width="1170" height="490" frameborder="0" style="border:0;" allowfullscreen="" aria-hidden="false" tabindex="0"></iframe>
			<div class="contact_map_info">
			<div class="contact_map_info__title"><?php esc_html_e( 'Phone', 'kmcentre' ); ?></div>
			<div class="contact_map_info__content">+38 (044) 272 11 75<br>+38 (095) 438 68 71<br>+38 (067) 186 18 70</div>
			<div class="contact_map_info__title"><?php esc_html_e( 'Email', 'kmcentre' ); ?></div>
			<div class="contact_map_info__content">kmcentre1@gmail.com</div>
			<div class="contact_map_info__title"><?php esc_html_e( 'Address', 'kmcentre' ); ?></div>
			<div class="contact_map_info__content">М. Київ,<br>Вул. Січових Стрільців (Артема), 5-Б, <br>оф. 23</div>

			</div>
		</div>
			
		</div>
	</div>
	



</div>
	
</main>
<!-- #main -->
<?php
get_footer();
